...
#'  "n.bins"		total number of distance bins (observed and unobserved)
#'  "n.obs.bins"  total number of bins within sighting strip
#'  "Bin.length"	vector giving relative size of distance bins 
#'  "Obs.bins"  Vector providing which of the n.bins are actual observable (i.e., within the strip)
...
#'  "move.formula" A formula specifying relationship between movement rate (or movement kernel SD) and potential covariates (e.g., Distance)
#'  "measure.formula" A formula specifying relationship between measurement error rate (or error kernel SD) and potential covariates (e.g., Distance)
#'  "move.family" A family for modeling the movement kernel.  Current choices are "gaussian" or "laplace" (double exponential)
#'  "measure.family" A family for modeling the measurement error kernel.  Current choices are "gaussian" or "laplace" (double exponential)
#'  "move.one.side"  If TRUE (default), movement can only be away from the transect line
#'  "move.cov" If provided (default is NULL) provides the name of the column in Dat corresponding to a binary individual covariate influencing whether individuals can move (cov=1) or not (cov=0)
#'  "first.obs.correct" If TRUE (FALSE is default), measurement error for the observer in the front seat is assumed zero (the variable "Seat" must be definied in Obs.cov and then observations w Seat==1 will have measurement error set to zero)
...
mcmc_ds<-function(Par,Data,cur.iter,adapt,Control,DM.hab.pois,DM.hab.bern=NULL,DM.det,Q,Prior.pars,Meta){	
  ...
  SMALL=10^{-20}
	...
	if(Meta$i.binned==1)dist.mult=1/(length(Meta$Obs.bins)-1)
	...
	Meta$move.cov.pl = NULL
	if(is.null(Meta$move.cov)==FALSE)Meta$move.cov.pl=which(Meta$stacked.names==Meta$move.cov)
	...
	Meta$dist.pl.true=Meta$dist.pl+1
	Meta$seat.pl=NULL
	if(Meta$first.obs.correct==TRUE)Meta$seat.pl=which(Meta$stacked.names=='Seat')
	...
	if(Meta$detect){
        ...
        #cat(paste("isp ",isp," itrans ",itrans))
				X=get_mod_matrix_det_move(Cur.dat=Data[isp,itrans,,],Meta$stacked.names,Meta$factor.ind,Meta$Det.formula,Meta$Levels,Meta$Obs.bins,Meta$n.bins)
				ExpY=X%*%Par$det
      ...
      Lambda[isp,]=exp(Par$Nu[isp,])*Meta$Area.hab  
			...
			if(Meta$detect & iiter>Control$iter.fix.N){
              ...
              #{
						...
						#}
							...
							X=get_mod_matrix_det_move(Cur.dat=Cur.dat,stacked.names=Meta$stacked.names,factor.ind=Meta$factor.ind,Det.formula=Meta$Det.formula,Levels=Meta$Levels,Meta$Obs.bins,Meta$n.bins)
							ExpY=X%*%Par$det
							P=c(1:a)
							for(i in 1:a){
                ...
                else Sigma[offdiag]=Par$cor*(Meta$i.binned*(Cur.dat[i*Meta$n.Observers[itrans]-1,Meta$dist.pl]-min(Meta$Obs.bins))*dist.mult+(1-Meta$i.binned)*Cur.dat[i*Meta$n.Observers[itrans]-1,Meta$dist.pl])
                if(!(Cur.dat[i*Meta$n.Observers[itrans]-1,Meta$dist.pl]%in%Meta$Obs.bins))Sigma[offdiag]=0
              ...
              tmp.sum=0
                ...
                Accept$N[isp,itrans]=Accept$N[isp,itrans]+1
								...
								if(Meta$n.Observers[itrans]>1){
									...
									Cor=Par$cor*(Meta$i.binned*(Dist[,1]-min(Meta$Obs.bins))*dist.mult+(1-Meta$i.binned)*Dist[,1])
                  ...
                  if(Meta$G.transect[isp,itrans]>0)Meta$N.transect[isp,itrans]=sum(Data[isp,itrans,1:n.Records[isp,itrans],Meta$stacked.names=="Group"])/Meta$n.Observers[itrans]
					...
					if(Meta$n.ind.cov>0){
					  for(icov in 1:Meta$n.ind.cov){
					    ...
					    Data[isp,itrans,(n.Records[isp,itrans]+1):Meta$M[isp,itrans],Meta$dist.pl+1+icov]=rep(rsamp,each=Meta$n.Observers[itrans])
					  }
					}
					
					#fill distances for unobserved (not in population)
					if(Meta$i.binned==1)Data[isp,itrans,(n.Records[isp,itrans]+1):Meta$M[isp,itrans],Meta$dist.pl.true]=rep(sample(c(1:Meta$n.bins),size=(Meta$M[isp,itrans]-n.Records[isp,itrans])/Meta$n.Observers[itrans],replace=TRUE,prob=Meta$Bin.length),each=Meta$n.Observers[itrans])
					...
					if(Meta$n.Observers[itrans]>1){#model movement for second observer, measurement error
					  #get indices for second observer rows
					  Second.index = c(1:((Meta$M[isp,itrans]-n.Records[isp,itrans])*0.5))*2 + n.Records[isp,itrans]
					  First.index = Second.index-1
					  Second.index.move=Second.index
					  First.index.move=First.index
					  Which.move.not.possible=NULL
					  if(!is.null(Meta$move.cov.pl)){  #establishing framework so that individuals with move.cov=0 can't move
					    Which.move.not.possible=which(Data[isp,itrans,,Meta$move.cov.pl]==0)
					    if(length(Which.move.not.possible)>0){
					      First.index.move=First.index.move[-which(First.index%in%Which.move.not.possible)]
					      Second.index.move=Second.index.move[-which(Second.index%in%Which.move.not.possible)]
					    }
					  }
					  Xmove=get_mod_matrix_move(Data[isp,itrans,First.index.move,],Meta$stacked.names,Meta$factor.ind,Meta$move.formula,Meta$Levels)
					  Move_pars = exp(Xmove%*%Par$beta.move)
						Data[isp,itrans,Second.index.move,Meta$dist.pl.true]=sample_movement(Par = Move_pars, Dist.true=Data[isp,itrans,First.index.move,Meta$dist.pl.true],n.bins=Meta$n.bins,move.family=Meta$move.family,move.one.side=Meta$move.one.side)
						Xmeasure=get_mod_matrix_move(Data[isp,itrans,(n.Records[isp,itrans]+1):Meta$M[isp,itrans],],Meta$stacked.names,Meta$factor.ind,Meta$measure.formula,Meta$Levels)
					  Measure_pars = exp(Xmeasure*Par$beta.measure)
					  Data[isp,itrans,(n.Records[isp,itrans]+1):Meta$M[isp,itrans],Meta$dist.pl]=sample_movement(Par = Measure_pars, Dist.true=Data[isp,itrans,(n.Records[isp,itrans]+1):Meta$M[isp,itrans],Meta$dist.pl.true],n.bins=Meta$n.bins,move.family=Meta$measure.family,move.one.side=FALSE)
  				}
					...
					if(cur.G>0){
						###   1) true distances
						Cur.indices=matrix((G.obs[isp,itrans]*Meta$n.Observers[itrans]+1):(Meta$G.transect[isp,itrans]*Meta$n.Observers[itrans]),nrow=2)
            Xmove=get_mod_matrix_move(Data[isp,itrans,Cur.indices[1,],],Meta$stacked.names,Meta$factor.ind,Meta$move.formula,Meta$Levels)
					  Par.move=exp(Xmove%*%Par$beta.move)
					  Xmeasure1=get_mod_matrix_move(Data[isp,itrans,Cur.indices[1,],],Meta$stacked.names,Meta$factor.ind,Meta$measure.formula,Meta$Levels)
					  Xmeasure2=get_mod_matrix_move(Data[isp,itrans,Cur.indices[2,],],Meta$stacked.names,Meta$factor.ind,Meta$measure.formula,Meta$Levels)
					  Par.measure1=exp(Xmeasure1%*%Par$beta.measure)
					  Par.measure2=exp(Xmeasure2%*%Par$beta.measure)
						Dist.old=matrix(Data[isp,itrans,(G.obs[isp,itrans]*Meta$n.Observers[itrans]+1):(Meta$G.transect[isp,itrans]*Meta$n.Observers[itrans]),Meta$dist.pl.true],ncol=2) #observer on rows
						Dist.obs=matrix(Data[isp,itrans,(G.obs[isp,itrans]*Meta$n.Observers[itrans]+1):(Meta$G.transect[isp,itrans]*Meta$n.Observers[itrans]),Meta$dist.pl],ncol=2) #observer on rows
						Dist.new=Dist.old+sample(c(1,0,-1),length(Dist.old),replace=TRUE)
					  Dist.new[which(Dist.new<1 | Dist.new>n.bins)]=Dist.old[which(Dist.new<1 | Dist.new>n.bins)]
					  #calculate likelihood of new distance 
					  L.old = log_lik_movement(Par.move,Dist.old[2,],Dist.old[1,],move.family=Meta$move.family)+  #movement likelhood
					    log_lik_movement(Par.measure1,Dist.obs[1,],Dist.old[1,],move.family=Meta$measure.family)+ #measurement error likelihood (obs 1)
					    log_lik_movement(Par.measure2,Dist.obs[2,],Dist.old[2,],move.family=Meta$measure.family) #measurement error likelihood (obs 2)
					  L.new = log_lik_movement(Par.move,Dist.new[2,],Dist.new[1,],move.family=Meta$move.family)+  #movement likelhood
					    log_lik_movement(Par.measure1,Dist.obs[1,],Dist.new[1,],move.family=Meta$measure.family)+ #measurement error likelihood (obs 1)
					    log_lik_movement(Par.measure2,Dist.obs[2,],Dist.new[2,],move.family=Meta$measure.family) #measurement error likelihood (obs 2)
					  Tmp.accept=(runif(ncol(Dist.old))<exp(L.new-L.old))
					  Dist.new=t(t(Dist.new)*Tmp.accept+t(Dist.old)*(1-Tmp.accept))
					  Data[isp,itrans,(G.obs[isp,itrans]*Meta$n.Observers[itrans]+1):(Meta$G.transect[isp,itrans]*Meta$n.Observers[itrans]),Meta$dist.pl.true]=Dist.new

					  ### 2) observed distances  
						Cur.dat=New.dat=Data[isp,itrans,(G.obs[isp,itrans]*Meta$n.Observers[itrans]+1):(Meta$G.transect[isp,itrans]*Meta$n.Observers[itrans]),]
						Dist.true=matrix(Cur.dat[,Meta$dist.pl.true],ncol=2)
						Obs.old=matrix(Cur.dat[,Meta$dist.pl],ncol=2)
						Obs.new=Obs.old+sample(c(1,0,-1),length(Obs.old),replace=TRUE)
						Obs.new[which(Obs.new<1 | Obs.new>n.bins)]=Obs.old[which(Obs.new<1 | Obs.new>n.bins)]
            New.dat[,Meta$dist.pl]=Obs.new
            #[Perceived dist | True dist] component
            Xmeasure1=get_mod_matrix_move(Data[isp,itrans,Cur.indices[1,],],Meta$stacked.names,Meta$factor.ind,Meta$measure.formula,Meta$Levels)
            Xmeasure2=get_mod_matrix_move(Data[isp,itrans,Cur.indices[2,],],Meta$stacked.names,Meta$factor.ind,Meta$measure.formula,Meta$Levels)
            Par.measure1=exp(Xmeasure1%*%Par$beta.measure)
            Par.measure2=exp(Xmeasure2%*%Par$beta.measure)
            L.old=log_lik_movement(Par.measure1,Obs.old[1,],Dist.true[1,],move.family=Meta$measure.family)+ #measurement error likelihood (obs 1)
              log_lik_movement(Par.measure2,Obs.old[2,],Dist.true[2,],move.family=Meta$measure.family) #measurement error likelihood (obs 2)
            L.new=log_lik_movement(Par.measure1,Obs.new[1,],Dist.true[1,],move.family=Meta$measure.family)+ #measurement error likelihood (obs 1)
              log_lik_movement(Par.measure2,Obs.new[2,],Dist.true[2,],move.family=Meta$measure.family) #measurement error likelihood (obs 2)
            #[Detection = 0 | Perceived dist] component
						Xold=get_mod_matrix_det_move(Cur.dat=Cur.dat,stacked.names=Meta$stacked.names,factor.ind=Meta$factor.ind,Det.formula=Meta$Det.formula,Levels=Meta$Levels,Meta$Obs.bins,Meta$n.bins)
						Xnew=get_mod_matrix_det_move(Cur.dat=New.dat,stacked.names=Meta$stacked.names,factor.ind=Meta$factor.ind,Det.formula=Meta$Det.formula,Levels=Meta$Levels,Meta$Obs.bins,Meta$n.bins)
            ExpY.old=Xold%*%Par$det
            ExpY.new=Xnew%*%Par$det
						...
						for(i in 1:cur.G){ #not set up for last.ind or continuous distance
						  Sigma[offdiag]=Par$cor*(Cur.dat[i*Meta$n.Observers[itrans]-1,Meta$dist.pl]-min(Meta$Obs.bins))*dist.mult
						  if(!(Cur.dat[i*Meta$n.Observers[itrans]-1,Meta$dist.pl]%in%Meta$Obs.bins))Sigma[offdiag]=0
						  L.old[i]=L.old[i]+dmvnorm(Tmp.Y.tilde[(i*Meta$n.Observers[itrans]-Meta$n.Observers[itrans]+1):(i*Meta$n.Observers[itrans])],mean=ExpY.old[(i*Meta$n.Observers[itrans]-Meta$n.Observers[itrans]+1):(i*Meta$n.Observers[itrans])],sigma=Sigma)
						  Sigma[offdiag]=Par$cor*(New.dat[i*Meta$n.Observers[itrans]-1,Meta$dist.pl]-min(Meta$Obs.bins))*dist.mult
						  if(!(New.dat[i*Meta$n.Observers[itrans]-1,Meta$dist.pl]%in%Meta$Obs.bins))Sigma[offdiag]=0
						  L.new[i]=L.new[i]+dmvnorm(Tmp.Y.tilde[(i*Meta$n.Observers[itrans]-Meta$n.Observers[itrans]+1):(i*Meta$n.Observers[itrans])],mean=ExpY.new[(i*Meta$n.Observers[itrans]-Meta$n.Observers[itrans]+1):(i*Meta$n.Observers[itrans])],sigma=Sigma)
						}
						Tmp.accept=(runif(ncol(Dist.true))<exp(L.new-L.old))
						Obs.new=t(t(Obs.new)*Tmp.accept+t(Obs.old)*(1-Tmp.accept))
						Data[isp,itrans,(G.obs[isp,itrans]*Meta$n.Observers[itrans]+1):(Meta$G.transect[isp,itrans]*Meta$n.Observers[itrans]),Meta$dist.pl]=Obs.new
		...
		if(Meta$misID){
          ...
          ExpY=X%*%Par$det
				...
				New.dat[,4]=prop.sp
				if(Meta$detect){
					X=get_mod_matrix_det_move(Cur.dat=New.dat,stacked.names=Meta$stacked.names,factor.ind=Meta$factor.ind,Det.formula=Meta$Det.formula,Levels=Meta$Levels,Meta$Obs.bins,Meta$n.bins)
					ExpY.prop=X%*%Par$det
				}
			...
			for(isp in 1:Meta$n.species){
				...
				if(n.gt0>0){
					for(itrans in 1:n.gt0){
						...
						X.temp=array(t(X=get_mod_matrix_det_move(Cur.dat=Cur.dat,stacked.names=Meta$stacked.names,factor.ind=Meta$factor.ind,Det.formula=Meta$Det.formula,Levels=Meta$Levels,Meta$Obs.bins,Meta$n.bins)),
						             dim=c(n.beta.det,Meta$n.Observers[GT0[itrans]],Meta$G.transect[isp,GT0[itrans]]))
					...
					for(isp in 1:Meta$n.species){
						...
						if(n.gt.one>0){
							for(itrans in 1:n.gt.one){
								...
								X.temp=array(t(X=get_mod_matrix_det_move(Cur.dat=Cur.dat,stacked.names=Meta$stacked.names,factor.ind=Meta$factor.ind,Det.formula=Meta$Det.formula,Levels=Meta$Levels,Meta$Obs.bins,Meta$n.bins)),
								             dim=c(n.beta.det,2,Meta$G.transect[isp,I.gt.one[itrans]]))
        ...
        for(itrans in 1:Meta$n.transects){
          for(isp in 1:Meta$n.species){
            if(Cur.G[isp,itrans]>0){
              ...
              if(Meta$detect)X.temp=get_mod_matrix_det_move(Cur.dat=Cur.dat,stacked.names=Meta$stacked.names,factor.ind=Meta$factor.ind,Det.formula=Meta$Det.formula,Levels=Meta$Levels,Meta$Obs.bins,Meta$n.bins)
  ...
  if(Meta$ZIP){
	...
	Det.names=colnames(get_mod_matrix_det_move(Cur.dat=matrix(Data[1,1,1,],nrow=1),stacked.names=Meta$stacked.names,factor.ind=Meta$factor.ind,Det.formula=Meta$Det.formula,Levels=Meta$Levels,Meta$Obs.bins,Meta$n.bins))
